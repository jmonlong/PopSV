...
```{r, message=FALSE}
dgv.cont3 = draw.controls.dist(dgv, list(gene=genes, gap=gap, sd=segdups), chr.prefix="chr", dist.gr=gap, nb.class=50)
```
...
mean(overlapsAny(dgv, gap))
mean(overlapsAny(dgv.cont3, gap))
mean(overlapsAny(dgv, segdups))
mean(overlapsAny(dgv.cont3, segdups))
```

Although the *overlap* with a gap is not exactly the same anymore, we now control for the *distance* to a gap, as shown in the next graphs.
