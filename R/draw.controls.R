...
##' @import GenomeInfoDb
##' @export
draw.controls <- function(cnv.gr, feat.grl, nb.class=20, nb.cores=3, redo.duplicates=TRUE, seed.nb.max=1e5, min.nb.gr=NULL){
  ...
  randGR.bp <- function(n=10){
    ...
    return(GenomicRanges::GRanges(chrs, IRanges::IRanges(starts, width=1)))
  }
  ...
  mcols(cnv.gr) = data.frame(as.data.frame(ol.l), sample=cnv.gr$sample)
  ol.prof = unique(mcols(cnv.gr)[,1:length(feat.grl), drop=FALSE])
  null.gr = parallel::mclapply(1:nrow(ol.prof), function(ii){
    gr.ii = cnv.gr[which(apply(t(as.matrix(mcols(cnv.gr)[,1:length(feat.grl), drop=FALSE]))==unlist(ol.prof[ii,]),2,all))]
    widths = width(gr.ii)
    ...
    tt = tapply(1:length(gr.ii), w.class, function(iii){
      ...
      good.d = apply(t(as.matrix(mcols(d.gr))-demi.w)*ifelse(unlist(ol.prof[ii,]),1,-1),2, function(x)all(x<0))
  ...
  if(redo.duplicates && any((dup = duplicated(as.data.frame(null.gr))))){
    redo.gr = draw.controls(null.gr[which(dup)], feat.grl, nb.class=nb.class, nb.cores=nb.cores, redo.duplicates=FALSE, min.nb.gr=NULL)
