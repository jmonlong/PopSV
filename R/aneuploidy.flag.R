...
aneuploidy.flag <- function(samp, files.df, col.file="bc.gz", nb.bins=1e3, prop.aneu=.1, plot=FALSE){
  ...
  if(plot){
    ...
    p1 = ggplot2::ggplot(df, ggplot2::aes(x=bc, fill=aneu.flag)) + ggplot2::geom_density(alpha=.4) + ggplot2::theme_bw() + ggplot2::facet_grid(chr~., scales="free") + ggplot2::xlab("raw read coverage") + ggplot2::theme(axis.text.y=ggplot2::element_blank())
    df$all = "all"
