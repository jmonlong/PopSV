...
##' @author Jean Monlong
##' @export
aneuploidy.flag <- function(samp, files.df, col.file = "bc.gz", nb.bins = 1000, prop.aneu = 0.1, 
    plot = FALSE) {
    ...
    df.sub = dplyr::do(dplyr::group_by(df, chr), {
        .[sample.int(nrow(.), nb.bins), ]
    })
    ...
    if (plot) {
        ...
        p1 = ggplot2::ggplot(df, ggplot2::aes(x = bc, fill = aneu.flag)) + ggplot2::geom_density(alpha = 0.4) + 
            ...
            ggplot2::xlab("raw read coverage") + ggplot2::ylab("bin density") + ggplot2::theme(axis.text.y = ggplot2::element_blank()) + 
            ggplot2::theme(legend.position = "bottom")
        df$all = "all"
        p2 = ggplot2::ggplot(df, ggplot2::aes(x = bc)) + ggplot2::geom_density(fill = "grey70", 
            alpha = 0.4) + ggplot2::theme_bw() + ggplot2::ylab("bin density") + ggplot2::xlab("raw read coverage") + 
