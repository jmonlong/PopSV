call.abnormal.cov.multiSamp <- function(z, samples, out.pdf = NULL, FDR.th = 0.05, 
    chunk.size = 1e+05) {
    ...
    if (is.character(z) & length(z) == 1) {
        ...
        while (length((lines = readLines(con, n = chunk.size))) > 0) {
            z.chunk = matrix(unlist(strsplit(lines, "\t")), ncol = length(headers), 
                byrow = TRUE)
            res.chunk = data.frame(chr = z.chunk[, 1], start = as.integer(z.chunk[, 
                2]), end = as.integer(z.chunk[, 3]), stringsAsFactors = FALSE)
        ...
        res.df = data.frame(z[, c("chr", "start", "end")], chi = sqrt(rowSums(z[, 
            samples] * z[, samples])), stringsAsFactors = FALSE)
        rm(z)
    }
    
    res.df = res.df[which(!is.na(res.df$chi)), ]
    ## Pvalue/Qvalue estimation
    if (all(is.na(res.df$chi))) 
        return(NULL)
    res.df$pv = pchisq(res.df$chi^2, df = length(samples), lower.tail = FALSE)
    if (any(res.df$pv < 1e-50)) 
        res.df$pv[res.df$pv == 0] = 1e-50
    ft = fdrtool::fdrtool(res.df$pv, statistic = "pvalue", plot = FALSE, verbose = FALSE)
    if (any(ft$qval < 0.05, na.rm = TRUE) | !any(res.df$pv < 1e-10)) {
        res.df$qv = ft$qval
    } else {
    ...
    if (!is.null(out.pdf) & any(!is.na(res.df$pv))) {
        ...
        print(ggplot2::ggplot(res.df, ggplot2::aes(x = pv)) + ggplot2::geom_histogram() + 
            ...
            ggplot2::theme_bw())
        print(ggplot2::ggplot(res.df, ggplot2::aes(x = ppoints(length(pv)), y = sort(pv))) + 
            ggplot2::geom_point() + ggplot2::scale_y_log10() + ggplot2::scale_x_log10() + 
            ggplot2::xlab("Expected P-value") + ggplot2::ylab("Observed P-value") + 
            ggplot2::theme_bw())
    }
