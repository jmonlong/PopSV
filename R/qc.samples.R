...
##' @author Jean Monlong
##' @export
qc.samples <- function(files.df, bin.df, ref.samples=NULL, nb.ref.samples=NULL, outfile.prefix, plot=TRUE, appendIndex.outfile=TRUE, chunk.size=1e5, col.bc="bc.gc.gz", nb.cores=1){
    if(is.null(ref.samples)) ref.samples = as.character(files.df$sample)
    if(is.null(nb.ref.samples)) nb.ref.samples = length(ref.samples)

    ## Flexible function to read bin counts on specific bins, several files, ...
    read.bc.samples <- function(df, files.df, nb.cores=1, med.med=NULL, med.samp=NULL, file=NULL, append.f=FALSE, sub.bc=NULL){
        ...
        if(!is.null(file)){
            write.table(bc.df, file=file, quote=FALSE, row.names=FALSE, sep="\t", append=append.f, col.names=!append.f)
        }
        if(!is.null(sub.bc)) bc.df=bc.df[sample(1:nrow(bc.df),min(c(nrow(bc.df),sub.bc))),]
        return(bc.df)
    }
    ...
    if(length(ref.samples)>nb.ref.samples){
        sparse.pts <- function(m, nb.pts){
            ...
            while(length(pts.ii)<nb.pts){
                m.ii = which.max(apply(dm[pts.ii, pts.jj, drop=FALSE], 2, min, na.rm=TRUE))
        ...
        ## Get subset of bins
        bc.rand = read.bc.samples(bin.df[sample(1:nrow(bin.df), 1e3),], files.df, med.med=1, med.samp=rep(list(1),nrow(files.df)))
        ## PCA
    ...
    if(nrow(bin.df)<1.3*chunk.size){
        ...
        analyze.chunk <- function(df){
            ...
            read.bc.samples(df.o, files.df, nb.cores, med.med, med.samp, file=outfile.prefix, append.f=ch.nb>1, sub.bc=chunk.size/nb.chunks)
        }
        if(is.null(bc.rand)){
            bc.rand = read.bc.samples(bin.df[sample(1:nrow(bin.df), 1e3),], files.df,med.med=1, med.samp=rep(list(1),nrow(files.df)))
        }
