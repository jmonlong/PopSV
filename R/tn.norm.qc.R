...
tn.norm.qc <- function(norm.stats, out.pdf="normStats-QC.pdf", bin.size=FALSE){
   ...
   print(ggplot2::ggplot(res.df, ggplot2::aes(x=d.max, y=nb.remove, fill=log10(..count..))) + ggplot2::stat_bin2d() + ggplot2::theme_bw() + ggplot2::xlab("correlation distance to last supporting bin") + ggplot2::ylab("number of outlier samples removed")  + ggplot2::scale_fill_gradient(name="log10(nb bins)",low="white", high="red"))
    ...
    if(bin.size){
        print(ggplot2::ggplot(res.df, ggplot2::aes(x=d.max, y=end-start, fill=log10(..count..))) + ggplot2::stat_bin2d() + ggplot2::theme_bw() + ggplot2::xlab("correlation distance to last supporting bin") + ggplot2::ylab("bin size (bp)") + ggplot2::scale_fill_gradient(name="log10(nb bins)",low="white", high="red"))
        print(ggplot2::ggplot(res.df, ggplot2::aes(x=nb.remove, y=end-start, fill=log10(..count..))) + ggplot2::stat_bin2d() + ggplot2::theme_bw() + ggplot2::xlab("number of outlier samples removed") + ggplot2::ylab("bin size (bp)") + ggplot2::scale_fill_gradient(name="log10(nb bins)",low="white", high="red"))
    }
    dev.off()
