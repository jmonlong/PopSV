...
quick.count <- function(files.df, bins.df, nb.cores = 1, col.files = NULL, ...) {
    if (is.null(col.files)) {
        ...
        bc.df = createEmptyDF(c(sapply(c("chr", "start", "end"), function(cn) class(bins.df[, 
            cn])), rep("integer", nrow(files.df))), nrow(bins.df))
    } else {
        if (nb.cores > 1) {
            bc.l = parallel::mclapply(files.df$sample, function(samp) {
        ...
        bc.df = createEmptyDF(c(sapply(c("chr", "start", "end"), function(cn) class(bins.df[, 
            cn])), rep("numeric", nrow(files.df))), nrow(bins.df))
    }
